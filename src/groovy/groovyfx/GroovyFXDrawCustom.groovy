...
start {
    stage(title: 'GroovyFX Draw', visible: true) {
        scene = scene(id: "sc", fill: WHITE, width: Screen.getPrimary().getVisualBounds().getWidth(), 
            height: Screen.getPrimary().getVisualBounds().getHeight()) {

            canvas = canvas(id: "drawcanvas", width: bind(sc.width()),
                height: bind(sc.height()))
            GraphicsContext graphicsContext = canvas.graphicsContext2D
            ...
            hbox(spacing: 10, padding: 10) {
                button("Reset", onAction: {reset(graphicsContext)})
                colorChooser = choiceBox(value:"Black", items: FXCollections.observableArrayList(
                ...
                sizeChooser = choiceBox(value: 1, items: FXCollections.observableArrayList(
                        1, 2, 3, 4, 5)){
                    onSelect {
                        control, item ->
